...
- Until that trust is removed, audit every Internet-facing floating workload
  for the private-route access it would inherit if scheduled on the Pi. Pin it
  away from the Pi or document a reviewed NetworkPolicy and route-level proof;
  do not copy floating placement into a new public service by default.
